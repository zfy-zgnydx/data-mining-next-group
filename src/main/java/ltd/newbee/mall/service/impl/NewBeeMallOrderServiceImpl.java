...
public class NewBeeMallOrderServiceImpl implements NewBeeMallOrderService {
    ...
    @Override
    public Long getLatestOrder(String username)
    {
        Long order=newBeeMallOrderMapper.selectByUserName(username);
        if(order==null) return 0L;
        NewBeeMallOrderItem good_id=newBeeMallOrderItemMapper.selectByOrderIdlimit(order);
        return good_id.getGoodsId();
    }
